...
class Game {
  play() {
    if (this.#isValidInput()) {
      console.log(
        'Please provide an odd number of unique moves (minimum of 3) \nExample: Rock Paper Scissors'
      );
      process.exit(0);
    }
    ...
    console.log(
      `HMAC: ${hmacCalculator.calculateHmac(secretKey, moves[computerMove])}`
    );
    this.displayMenu();
  ...
  handleUserInput() {
    while (true) {
      ...
      if (move === '0') {
        ...
        console.log(
          `Please enter a move number between 1 (${moves[0]}) and ${N} (${
            moves[N - 1]
          }).`
        );
      }
  ...
  displayKey(secretKey) {
    ...
    console.log(
      `To verify fair play, visit https://www.devglan.com/online-tools/hmac-sha256-online`
    );
  }

  #isValidInput() {
    return N < 3 || !(N & 1) || new Set(moves).size != N;
  }
...
class KeyGenerator {
  generateRandomKey(keyLength = 256) {
    return randomBytes(keyLength / 8).toString('hex');
  }
}
